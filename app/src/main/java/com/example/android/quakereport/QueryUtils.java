...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() throws JSONException {
        ...
        try {
            ...
            JSONArray earthquakeArray = new JSONArray(root.getJSONArray("features"));

            for(int i = 0; i <= earthquakeArray.length(); i++){
                JSONObject firstQuake = earthquakeArray.getJSONObject(i);
                JSONObject propQuake = firstQuake.getJSONObject("properties");
                double magnitude = propQuake.getDouble("mag");
                String place = propQuake.getString("place");
                int time = propQuake.getInt("time");
